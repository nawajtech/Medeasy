...
export const menuItems = [
  ...
  { to: "/departments", label: "Departments", permission: PERMISSIONS.DEPARTMENT_VIEW, section: "clinic", tenantModule: "clinic" },
...
export function filterMenuByPermissions(permissions = [], role, companyModules = null) {
  ...
  return menuItems.filter((item) => {
    ...
    if (item.tenantModule && !isSuperAdmin && !modules.has(item.tenantModule)) return false;
    if (!set.has(item.permission)) return false;
...
export function canAccessRoute(permissions = [], role, path, companyModules = null) {
  ...
  for (const rule of routeRules) {
    if (rule.pattern.test(path)) {
      ...
      return set.has(rule.permission);
    }
  }
